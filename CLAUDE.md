...
**Multiple pedidos per offer.** An offer can carry several pedidos. The data model mirrors the notas-fiscais pattern: **`reg.pedido` stays the FIRST pedido** (so `alertas.js`, `dashboard.js`, `schema.js` and old exports keep reading it unchanged) and **`reg.pedidos` is the array with ALL of them**. `getPedidosRegistro(reg)` is the single reader — it returns `reg.pedidos` when present and falls back to `[reg.pedido]`, so **records saved before this feature (and Excel imports, which only set `pedido`) work without migration**. In the form, `#blocoPedidos` holds one `.pedido-item` per pedido; item 1 keeps the original fixed IDs (`numero_pedido`, `valor_pedido`, …) and items 2+ are built by `_pedidoTemplateHtml()` using only classes (`.ped-numero`, `.ped-valor`, …) — collection reads by class scoped to each item (`coletarPedidosDoForm`), so both paths are uniform. Radio names are re-suffixed per index in `reindexarPedidos()` (`entregue_2`, `sol_oc_2`) — without that, one pedido's radio group would deselect another's. Each pedido has its own NF block (`.bloco-nfs`); `preencherNotasFiscaisUI(pedido, containerEl)` takes an optional container — omitting it targets pedido 1's `#blocoNotasFiscais` (which keeps the fixed NF IDs), while pedidos 2+ pass their own block and emit no IDs (avoids duplicates). The "Adicionar NF" of pedidos 2+ is handled by delegation on `#blocoPedidos` that skips `#btnAdicionarNF` (pedido 1 has its own handler in `initNotasFiscaisUI`). Modal renders one section per pedido. **Exports:** the schema-driven ones go through `buildSchemaComNotas(schemaBase, maxNFs, maxPedidos)`, which injects the extra-pedido columns (`qtde_pedidos`, `ped_<n>_<campo>` → labels `Qtde Pedidos`, `P2 Nº Pedido`, …) right after `pedido.data_implantacao`, so they stay grouped with the pedido block instead of dangling at the end — same mechanism the NF columns already used (inserted after `pedido.obs`). Values resolve in `_valorColunaPedidoExtra()`. Only `exportExcel` passes `maxPedidos` today; the two PDF exports call it with the default `1` (12 extra columns per pedido would wreck the PDF layout) but already carry the resolution branch, so enabling them is a one-argument change. The backup export (`json_to_sheet` of `ofertasData`) is hand-built and instead carries `QtdePedidos` + `PedidosAdicionais` (text summary via `formatarPedidosExtrasTexto`). **The count of pedidos is per-record, so these columns can never be static entries in `schema.js`** — same reason NF columns aren't.
