...
function verRepresentada(id) {
    ...
    let html = `
        <div class="modal-section">
            <strong>Nome da Representada:</strong> ${rep.nome || "-"}
        </div>
        <div class="modal-section">
            <strong>Ofertas vinculadas:</strong> ${qtdOfertas}
        </div>
        <hr>
        <div class="modal-section">
            <strong>Usuário :</strong> ${usuario}
        ...
        try {
            ...
            if (shOfertas) {
                ...
                registros = dadosOf
                    ...
                    .map(row => {
                        const pedido = (row.NumeroPedido || row.ValorPedido || row.RefProjetoPedido || row.TipoProduto || row.CondicaoPagamento || row.Obs)
                            ? {
